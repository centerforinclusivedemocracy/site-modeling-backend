...
# ACS
dat = read.csv(paste0(root, "data/output/ACS_Indicators_TractsTX.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character", "FIPS"="character"))
...
# job
job = read.csv(paste0(root, "data/output/JobShare_Block_blocksTX.csv"), stringsAsFactors = FALSE, colClasses = c("w_geocode"="character", "w_FIPS"="character"))
colnames(job)[1:2] <- c("GEOID10", "FIPS")
job = job[,c(1,2,5)]
head(job)

# pop dens
pop = read.csv(paste0(root, "data/output/PopDensity_Block_blocksTX.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID10"="character", "TRACTCE10"="character", 
                                                                                                                   "COUNTYFP10"="character"))
pop = pop[,c(5, 2, 19, 20, 22, 23)]
colnames(pop)[2:3] <- c("FIPS", "pop2010")
pop$FIPS = paste0("48", pop$FIPS)
head(pop)


# elig
elig = read.csv(paste0(root, "data/output/Elig_NonReg_Pop_TractsTX.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID"="character", "FIPS"="character"))
...
# poll
poll = read.csv(paste0(root, "data/output/PollShare_BlocksTX.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID10"="character", "FIPS"="character"))
colnames(poll)[1] <-"GEOID10"
head(poll)
summary(poll)

# vbm rate
vbm = read.csv(paste0(root, "data/output/VBM_Use_Rate_BlocksTX.csv"), stringsAsFactors = FALSE, colClasses = c("GEOID10"="character", "FIPS"="character"))
...
write.csv(dat, paste0(outputRoot, "model data/indicators_Stand_Final_TX.csv"), row.names = FALSE)
...
# load blocks 
poly.bg.df = read.dbf("C:/Users/lauradal/Box/CCEP Files/Siting Tool/data/decennial/blocksCA.dbf", as.is = TRUE)
...
test_poly.bg.df= read.dbf("C:/Users/lauradal/Box/CCEP Files/Siting Tool/data/decennial/blocksCA.dbf", as.is = TRUE)
